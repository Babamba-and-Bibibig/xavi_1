...
- `serve --launch-chrome` 는 dedicated Chrome frontend 를 띄우는 편의 기능이며, 현재 CDP 책임은 dev-console target readiness 확인까지다.
- Chrome CDP page event/WebSocket attach, 외부 target action capture, real `spawn_agent` bridge 는 별도 후속 기능으로 분리한다.
...
- source projection: `.xavi/development_trace.sqlite3` 를 HTML, report JSON, SSE role lanes 로 투영한다.
- Chrome launch: `serve --launch-chrome --remote-debugging-port <port>` 로 dedicated Chrome profile/frontend 를 띄울 수 있다.
- CDP status endpoint: `GET /api/cdp/status`
- no-launch status: `not_launched`
- launched positive status: `target_ready` when `/json/version` succeeds and the dev-console target appears in `/json/list`.
- explicit port mode: `DevToolsActivePort` 는 진단 필드다. readiness gate 로 사용하지 않는다. 따라서 `target_ready` 는 `active_port_file_seen:false` 와 함께 반환될 수 있다.
...
## CDP Regression Notes

다음 버그는 이미 수정된 회귀로 취급한다.

- stale launch-time `CdpStatus`: `/api/cdp/status` 는 launch 시점 값을 재사용하지 않고 요청 시점에 live probe 한다.
- explicit-port `DevToolsActivePort` gate: 명시 `--remote-debugging-port` 모드에서 active port file 부재는 실패 조건이 아니다.
- fragile 250ms raw CDP probe: 현재 CDP HTTP probe 는 더 긴 bounded read 를 사용한다.

현재 테스트가 고정하는 CDP 경로:

- no-ActivePort `target_ready`
- delayed CDP response
- `/json/version` failure
