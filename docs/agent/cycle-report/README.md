...
- 보고서는 요약 중심 산출물이 아니라 원문 evidence 우선 산출물로 만든다. 역할 지시/반환, 테스트 원문, 제외된 코드 변경 사유, audit failure/missing evidence 를 `index.html` 의 기본 노출 근거로 두고, 전체 diff hunk 는 `diff.html` 에서 전용으로 렌더링한다.
...
├── index.html
├── diff.html                 # optional direct artifact
├── report.json
├── raw.json
├── audit.json
└── context.md
...
- `index.html`: 사용자가 브라우저에서 보는 정적 HTML 보고서. 작업 흐름, 상단 evidence/audit 경고, 사용자 요청, 역할별 dispatch/return evidence, command evidence, changed_files 색인, raw/context drill-down 을 중심으로 보여준다.
- `diff.html`: 선택 산출물. `report.json.code_changes[]` 의 파일별 hunk/line evidence 를 전체 렌더링하는 diff 전용 정적 HTML 보고서
...
`index.html` 과 `diff.html` 은 browser viewer 가 직접 열 수 있는 prebuilt artifact 다.
dev-console 의 canonical `/reports/<cycle_id>/` route 는 `index.html` 을 직접 서빙해야 하며,
canonical `/reports/<cycle_id>/diff.html` route 는 `diff.html` 이 존재할 때 이 파일을 직접 서빙해야 한다.
`/api/reports/<cycle_id>/diff.html` 도 direct artifact API 로 같은 파일을 서빙할 수 있다.
큰 `report.json` 또는 raw artifact 를 다시 읽어 HTML 을 합성하다가 대형 artifact 오류로 실패하는 경로가 아니다.
prebuilt HTML 직접 서빙을 지원할 때는 reports root/cycle dir canonical containment,
`index.html`/`diff.html` symlink 거부, required artifact symlink escape 거부가 함께 유지되어야 한다.

required core artifact 는 `index.html`, `report.json`, `raw.json`, `audit.json`, `context.md` 이다.
allowed direct artifact 는 required core 에 `diff.html` 을 더한 목록이다.
`diff.html` 은 새 optional artifact 이므로 기존 old artifact 에 이 파일이 없어도 `/reports/<cycle_id>/`,
`/api/reports/<cycle_id>/ready`, core readiness 검증이 실패하면 안 된다.
readiness JSON 은 core 준비 여부와 별개로 `diff_html_present` boolean 을 노출한다.
...
- `code_changes`: 파일별 코드 변경 hunk, 라인 배열, 한국어 설명 배열. 전체 line 렌더링은 `diff.html` 전용이며 `index.html` 은 파일/hunk anchor 색인과 diff page 링크만 보여준다.
- `commands`: 명령, actor, result, evidence 배열. 원문 필드가 있으면 `command_verbatim`, `result_verbatim`, `output_verbatim` 을 서로 독립된 evidence 로 둔다.
- `artifacts`: `index.html`, 선택 `diff.html`, `report.json`, `raw.json`, `audit.json`, `context.md`, `latest.json` 경로 객체
...
- 테스트 명령, 결과, 출력 원문
- `report.json.code_changes[]` 의 파일/hunk anchor 색인과 `diff.html` 링크
- `audit.json.excluded_code_changes[]` 의 파일별 제외 사유

`diff.html` 은 `report.json.code_changes[]` 의 전체 hunk/line evidence 를 담당한다.
파일 목록, hunk navigation, full-width code viewport, horizontal scroll, CSS-only wrap toggle,
old/new line number gutter, 파일/hunk metadata, hunk explanations 를 제공한다.
기본 code line 표시는 `white-space: pre` 와 `overflow-x: auto` 이며 긴 줄을 숨기지 않는다.
...
`index.html` 은 `changed_files` 와 `code_changes` 의 파일/hunk anchor 색인을 표시하고,
전체 diff line 렌더링은 `diff.html` 로 분리한다.
...
- `templates/diff.html`: diff 전용 정적 HTML 보고서 템플릿
